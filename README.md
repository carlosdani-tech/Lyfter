...
- Admin-only user management endpoints for listing, detail, update, and safe deactivation.
- Product CRUD for admins and product browsing for authenticated users.
- Active cart creation, item add/update/remove, abandoned cart reactivation, totals, and stock validation.
- Sales checkout from the authenticated client's cart with billing address and safe payment metadata.
