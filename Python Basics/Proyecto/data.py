...
def build_student_from_row(row, row_number, existing_students):
    """Convert one CSV row into a validated student dictionary."""
    ...
    spanish_grade = parse_grade_value(row[2], "spanish_grade", row_number)
    english_grade = parse_grade_value(row[3], "english_grade", row_number)
    social_studies_grade = parse_grade_value(
        row[4], "social_studies_grade", row_number
    )
    science_grade = parse_grade_value(row[5], "science_grade", row_number)
